...
function ListJobs() {
   ...
   const [selectedElement, setSelectedElement] = useState(null);
    ...
    const [myapplication,setMyApplications] = useState([]);
   useEffect(() => {
       ...
       fetch("http://3.93.52.148/api/jobs").then(response => response.json()).then(data => {
        setJobs(data)
        console.log(data)
       })

       fetch("http://3.93.52.148/api/jobs/applied/"+localStorage.getItem("id")).then(response => response.json()).then(data => {
                  ...
                  ? jobs.map((item, index) => (
                    ...
                    <div className="jobbox">
                      <div className="jobsec1 flex">
                          <img className="picLogo" src={"http://3.93.52.148/api/uploadjobpic/"+item.pic} width={200} height={200}/>
                       </div>
...
<div class="modal-dialog modal-lg">
    ...
    <div class="modal-header">
      ...
      <div class="container-fluid">
                ...
                {
                  (myapplication.length == 0) ? (<h1>No Jobs have had been applied</h1>): (
                  ...
                  <div className="flex myappimg">
                    <img className="picLogo"src={"http://3.93.52.148/api/uploadjobpic/"+item.pic} width={100} height={100}/>
                </div>
                ...
                <div class="col">
                    ...
                    <h5>Role :  <span>{item.job_title}</span></h5>
                    <h5>Location : {item.location}</h5>
                    <h5>Applied on: {formatDateAgo(item.application_date)}</h5>
                    <Link to={"/review/"+item.job_id}><button className="btn btn-warning">Review Application</button></Link>
                </div>
                 </div>
